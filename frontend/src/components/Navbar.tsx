      ...
      <div className="mx-auto flex max-w-[1400px] items-center gap-10 px-8 py-5">
        ...
        >
          {SECTIONS.map((s) => {
            ...
            const activePillClass = overVideo
              ? "bg-white/15 ring-1 ring-white/25"
              : "bg-black/[0.08] ring-1 ring-black/[0.12]";
            const textClass = overVideo
              ? isActive ? "text-white" : "text-white/60 hover:text-white"
              : isActive ? "text-sap-50" : "text-sap-300 hover:text-sap-50";
            return (
              <a
                key={s.id}
                href={`${onLanding ? "" : "/"}#${s.id}`}
              ...
              >
                {isActive && (
                  <span
                    className={`absolute inset-0 rounded-full transition-colors duration-300 ${activePillClass}`}
                  />
                )}
                <span className={`relative z-10 transition-colors ${textClass}`}>
                  {s.label}
                </span>
              </a>
            );
        ...
        <div className="ml-auto flex items-center gap-3">
          <div
            className={`hidden lg:flex items-center gap-3 rounded-full px-4 py-2 w-64 transition-colors duration-300 ${
              overVideo
                ? "bg-white/10 ring-1 ring-white/15"
                : "bg-black/[0.04] ring-1 ring-black/[0.08]"
            }`}
          >
            <Search
              size={14}
              className={overVideo ? "text-white/55" : "text-sap-400"}
            />
            <input
              type="text"
              placeholder="Search…"
              className={`w-full bg-transparent text-xs outline-none ${
                overVideo
                  ...
                  : "text-sap-50 placeholder-sap-400"
              }`}
            />
          </div>
